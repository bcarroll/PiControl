...
def create_database(database_file='db/PiControl.db'):
    ...
    if os.path.isfile(database_file):
        ...
        print("\n")
        print('######################################################################################')
        print('If this is the first (or only) instance of PiControl on the network, enter a new passphrase (8 characters minimum) to be used as the PiControl Secret Key.')
        print("\n")
        print('If you have already created a PiControl Secret Key passphrase on another Raspberry Pi on your network, enter that passphrase.')
        print('######################################################################################')
        print("\n")
...
def create_config(cursor, secret_key):
    ...
    try:
        ...
        try:
            cursor.execute("INSERT INTO 'config' ('id', 'beacon_port', 'beacon_interval', 'secret_key', 'log_level', 'log_file', 'log_files_backup', 'log_file_size', 'beacon_listener_enabled', 'beacon_sender_enabled', 'background_charts_enabled') VALUES ('active', 31415, 300, " + "'{}'".format(secret_key) + ", 30, 'logs/PiControl.log', 5, 4096000, 1, 1, 0)")
        except Exception as e:
